...
<table class="table nowrap table-bordered compact hover display" id="tbdocente_ac">
        <thead>
            <tr>
                <th>Cedula</th>
                <th>Nombre</th>
...
<script>
        ...
        $(document).ready(function(){
            ...
            $('#tbdocente_ac').mouseover(function(){
                //Al poner el cursor encima cambia el borde a rojo
                $('#tbdocente_ac').css("cursor","Pointer");
            }); //Asimismo debemos crear el evento mouseout para que vuelva a ponerse negro  
        });
        $(document).ready(function () {
             var table = $('#tbdocente_ac').DataTable();
                $('#tbdocente_ac tbody').on('click', 'tr', function () {
                    var data = table.row(this).data();
                   // alert('You clicked on ' + data[0] + "'s row");
                    $('#cedula_ac').val('');  //limpia el input
                    $('#cedula_ac').val(data[0]);
